...
.post(function(req, res, next) {
    var admins;
    admins = req.body['text'].split(/[\r\n]+/);
    base.file.write(base.config.confPath + 'admin.json', JSON.stringify(admins));
    res.render('tool-admin', {admins: admins.join('\n')});
})
.get(function(req, res, next) {
    var admins = JSON.parse(base.file.read(base.config.confPath + 'admin.json') || '[]');
    res.render('tool-admin', {admins: admins.join('\n')});
});
