...
function exportPaid(sheet, list, callback) {
    ...
    for (var i = 0; i < list.length; i++) {
        order = list[i];
        column = 1;
        sheet.Cell(row, column++).String(order.user.realname || '');
        sheet.Cell(row, column++).String(order.user.company || '');
        sheet.Cell(row, column++).String(order.user.position || '');
        if (order.planAmount === 0) {
            sheet.Cell(row, column++).Number(order.amount);
        } else {
            sheet.Cell(row, column++).Number(order.planAmount);
        }
        sheet.Cell(row, column++).Number(order.amount);
        sheet.Cell(row, column++).String(order.user.mobile || '');
        sheet.Cell(row, column++).String(order.user.email);
        sheet.Cell(row, column++).String(order.describe || '');
        sheet.Cell(row, column++).String(states[order.state]);
        sheet.Cell(row, column++).String(order.user.username.toString());
        sheet.Cell(row, column++).String(order._id.toString());
        sheet.Cell(row, column++).Date(order.added);
        sheet.Cell(row, column++).String(order.bankType || '');
        sheet.Cell(row, column++).String(order.payId || '');
        sheet.Cell(row, column++).String(order.payType || '');
        row++;
    }
    sheet.Cell(row, 4).Format.Font.Bold();
    sheet.Cell(row, 4).String('认购总额');
    sheet.Cell(row, 5).Format.Font.Bold();
    sheet.Cell(row, 5).Formula('SUM(E2:E' + (row - 1) + ')');
    deferred.resolve({
        'row': row,
        'column': column
    });
...
router.route('/new')
    ...
    .get(function (req, res, next) {
        res.render('project-new', {
            ...
            category: base.config.plug.category,
            badge: base.config.plug.badge,
...
router.route('/:id/workshop/:wid')
        ...
        base.Workshop
            ...
            .then(workshop => {
                workshop
                    ...
                    .then(result => {
                        console.log(result);
                        res.redirect(`/project/${curId}/workshop/${adviser._id}`);
                    })
                    .then(undefined, err => {
                        console.log(err);
                        res.redirect(`/project/${curId}/workshop`);
                    })
            })
            .then(undefined, err => {
                console.log(err);
    ...
    .get((req, res, next) => {
        ...
        base.Workshop
            ...
            .then(workshop => {
                res.render('project-workshop-detail', {
                    workshop: workshop,
                    projectId: curId
                })
            })
...
router.route('/:id/orders/paid/export')
    .get(function (req, res, next) {
        ...
        var headers = ['真实姓名', '公司', '职位', '拟认购额', '实际认购金额', '手机', '邮箱',
            '理由', '状态', '用户名', '订单号', '下单时间', '银行', '流水号', '付款类型'
        ];
        var statisticsHeaders = ['应付款', '已付款', '未付款', '应付款人数', '已付款人数', '未付款人数' ];
        for (var i = 0; i < headers.length; i++) {
            allSheet.Cell(1, i + 1).String(headers[i]);
            allSheet.Cell(1, i + 1).Format.Font.Bold();
            paidSheet.Cell(1, i + 1).String(headers[i]);
            paidSheet.Cell(1, i + 1).Format.Font.Bold();
            unpaidSheet.Cell(1, i + 1).String(headers[i]);
            unpaidSheet.Cell(1, i + 1).Format.Font.Bold();
        }
        for (i = 0; i < statisticsHeaders.length; i++) {
            statisticsSheet.Cell(1, i + 1).String(statisticsHeaders[i]);
            statisticsSheet.Cell(1, i + 1).Format.Font.Bold();
        }
        Q.all(promises)
            .then(function (result) {
                statistics.payable = result[0];
                ...
                statisticsSheet.Cell(2, 1).Number(statistics.payable.sum);
                statisticsSheet.Cell(2, 2).Number(statistics.paid.sum);
                statisticsSheet.Cell(2, 3).Number(statistics.payable.sum - statistics.paid.sum);
                statisticsSheet.Cell(2, 4).Number(statistics.payable.count);
                statisticsSheet.Cell(2, 5).Number(statistics.paid.count);
                statisticsSheet.Cell(2, 6).Number(statistics.payable.count - statistics.paid.count);
                return true;
            })
