...
.get(function(req, res, next) {
    ...
    base.Order.ordersByProject(curId, function(err, result) {
        ...
        var lines = [],
            headers = ['订单号', '额度', '状态', '用户名','真实姓名', '邮箱', '手机', '公司', '职位', '理由', '下单时间'];
        lines.push(headers.join('\t'));
        while(order = result.pop()) {
            var fields = [], why = (order.describe || '');
            fields.push(order._id);
            fields.push(order.amount);
            fields.push(order.state);
            ...
            fields.push(why.replace('\t', ' ').replace(/[\r\n]+/g, ''));
            fields.push(util.dateformat(order.added));
