...
.get(function(req, res, next) {
    ...
    base.User.find({isInvestor: true}, function(err, result) {
        ...
        while(user = result.pop()) {
            r++;
            c = 1;
            ws.Cell(r, c++).String(user.username || '');
            ws.Cell(r, c++).String(user.realname || '');
            ws.Cell(r, c++).String(user.email || '');
            ws.Cell(r, c++).String(user.mobile || '');
            ws.Cell(r, c++).String(user.company || '');
            ws.Cell(r, c++).String(user.position || '');
        }
